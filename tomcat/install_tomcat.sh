...
#cd /tmp
if [[ -f /tmp/apache-tomcat-9.0.45.tar.gz ]]; then
  echo "Skipping download of apache-tomcat-9.0.45.tar.gz"
else
  wget -O /tmp/apache-tomcat-9.0.45.tar.gz https://downloads.apache.org/tomcat/tomcat-9/v9.0.45/bin/apache-tomcat-9.0.45.tar.gz
fi
# extract the contents
rm -rf "$DEST_LOC"
mkdir "$DEST_LOC"
sudo tar xzvf /tmp/apache-tomcat-9.0.45.tar.gz -C /opt/tomcat --strip-components=1
